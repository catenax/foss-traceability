...
/**
 * Accept an access request for your private data collection
 * @param req
 * @param res
 * @param next
 * @constructor
 */
export default async function AcceptAccessRequest(req: any, res: any, next: any) {
  const client = new AccessMgmtClient();
  const jwt = JWT.parseFromHeader(req.header("Authorization"));

  return await client
    .acceptAccessRequest(req.body, JWT.parseMspIDFromToken(jwt))
    .then((response: any) => {
 ...
 * @ignore
 * @swagger
 * /v1/access-mgmt/accept-access-request:
 *   post:
 *     security:
 *       - Bearer: []
 *     description: Accept an access request for your private data collection
 *     tags: ['access-mgmt']
 *     produces:
 *       - application/json
 *     parameters:
