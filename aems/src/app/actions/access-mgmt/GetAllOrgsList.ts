...
/**
 * Get all orgs in PartChain
 * @param req
 * @param res
 * @param next
 * @constructor
 */
export default async function getAllOrgs(req: any, res: any, next: any) {
  const client = new AccessMgmtClient();
  const jwt = JWT.parseFromHeader(req.header("Authorization"));

  return await client
    .getAllOrgList(JWT.parseMspIDFromToken(jwt))
    .then((response: any) => {
 ...
 *     security:
 *       - Bearer: []
 *     description: Get all orgs in PartChain
 *     tags: ['access-mgmt']
 *     produces:
 *       - application/json
 *     responses:
