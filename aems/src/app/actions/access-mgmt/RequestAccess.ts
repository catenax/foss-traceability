 ...
 * @constructor
 */
export default async function RequestAccess(req: any, res: any, next: any) {
  const client = new AccessMgmtClient();
  const jwt = JWT.parseFromHeader(req.header("Authorization"));

  return await client
    .requestAccess(req.body, JWT.parseMspIDFromToken(jwt))
    .then((response: any) => {
 ...
 * @ignore
 * @swagger
 * /v1/access-mgmt/request-access:
 *   post:
 *     security:
 *       - Bearer: []
 *     description: Request access for a partner given a mspID
 *     tags: ['access-mgmt']
 *     produces:
 *       - application/json
 *     parameters:
