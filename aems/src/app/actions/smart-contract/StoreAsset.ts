...
/**
 * Store asset into the Ledger action
 * @param req
 * @param res
 * @param next
 * @constructor
 */
export default async function StoreAsset(req: any, res: any, next: any) {
  ...
  return client
    .upsertAsset(req.body.asset, JWT.parseMspIDFromToken(jwt))
    .then((response: any) => {
 ...
 * @ignore
 * @swagger
 * /v1/smart-contract/store-asset:
 *   post:
 *     security:
 *       - Bearer: []
 *     description: Query an asset from the ledger by given filters
 *     tags: ['smart-contract']
 *     produces:
 *       - application/json
 *     parameters:
