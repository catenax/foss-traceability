...
/**
 * Update asset into the Ledger action
 * @param req
 * @param res
 * @param next
 * @constructor
 */
export default async function UpdateAsset(req: any, res: any, next: any) {
  ...
  return client
    .updateAsset(req.body.asset, JWT.parseMspIDFromToken(jwt))
    .then((response: any) => Response.json(res, response, 200))
    .catch((error: any) => Response.json(res, Response.errorPayload(error), Response.errorStatusCode(error)));
}
/**
 * @ignore
 * @swagger
 * /v1/smart-contract/update-asset:
 *   post:
 *     security:
 *       - Bearer: []
 *     description: Update assets in the ledger
 *     tags: ['smart-contract']
 *     produces:
 *       - application/json
 *     parameters:
