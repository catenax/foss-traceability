...
import PartsQualityAlertModel from "../models/PartsQualityAlert";
...
import AlertModel from "../models/QualityAlert";
import AlertHistoryModel from "../models/AlertHistory";
...
export default class OffChainDBClient extends SQLClient {
   ...
   * @param mspID
   */
  async connectAndSync(mspID: string): Promise<{
    pool: Sequelize;
    assetModel: any;
    relationshipModel: any;
    partQualityAlertModel: any;
    eventModel: any;
    commentModel: any;
    alertModel: any;
    alertHistoryModel: any;
  }> {
    ...
    const partQualityAlertModel = await this.initModel(PartsQualityAlertModel, connectionPool);
    await partQualityAlertModel.sync();
    const eventModel = await this.initModel(EventModel, connectionPool);
    await eventModel.sync();
    const commentModel = await this.initModel(CommentModel, connectionPool);
    await commentModel.sync();
    const alertModel = await this.initModel(AlertModel, connectionPool);
    await alertModel.sync();
    const alertHistoryModel = await this.initModel(AlertHistoryModel, connectionPool);
    await alertHistoryModel.sync();
    return {
      pool: connectionPool,
      assetModel,
      relationshipModel,
      partQualityAlertModel,
      eventModel,
      commentModel,
      alertModel,
      alertHistoryModel,
  ...
  async getAssetDetail(
    serialNumberCustomers: string | Array<string>,
    maxChildLevel: number = Number(env.childrenMaxRecursiveLimits.list),
    mspIDFromJWT: string,
  ) {
    ...
    return connectionPool.pool
      .query(queryString, this.createRawQueryOptionsWithReplacement({ serialNumberList: serialNumberList }))
                                    ...
                                    (
                                         ...
                                         FROM (SELECT * from relationships where child_serial_number_customer = (SELECT serial_number_customer from assets where serial_number_customer ${whereConditionString})) Relationships --Only get the relevant relationship 
                                                  LEFT OUTER JOIN (SELECT  ${
                                                    fieldsRightOuterJoin ? `${fieldsRightOuterJoin.join(", ")}` : "*"
                                                  } from Assets) Assets 
  ...
  async storeAssets(assets: any[], mspIDFromJWT: string, mspID: string, relationshipExtras: any = Object.create({})) {
    ...
    assets = assets.map((tx: any) => Objects.mapKeys(tx, Strings.camelCaseToSnakeCase));

    const relationships = [];
    // First create relationships array
    for (let i in assets) {
      if (
        assets[i].hasOwnProperty("serial_number_customer") &&
        assets[i].hasOwnProperty("components_serial_numbers") &&
        assets[i]["components_serial_numbers"] != null
      ) {
        for (let child of assets[i]["components_serial_numbers"]) {
          if (child != null) {
            relationships.push({
              ...
              parent_mspid: mspID,
              child_mspid:
                relationshipExtras.hasOwnProperty(child) && relationshipExtras[child].hasOwnProperty("child_mspid")
                  ? relationshipExtras[child]["child_mspid"]
                  : "",
              transfer_status:
                relationshipExtras.hasOwnProperty(child) && relationshipExtras[child].hasOwnProperty("transfer_status")
                  ? relationshipExtras[child]["transfer_status"]
                  : 0,
  ...
  async updateAsset(assetsUpdated: any, mspIDFromJWT: string, relationshipExtras: any = Object.create({}), createTransactionRecord: boolean = false) {
    ...
    for (let partUpdated of assetsUpdated) {
      ...
      if (assetToUpdate) {
        ...
        const keysToOmit = _.difference(Object.keys(partUpdated), Object.keys(getAssetModelDefinition(AssetModel)));
        Logger.debug(`Key to omit: ${keysToOmit.toString()}`);
        const diff: any = _.fromPairs(
          _.differenceWith(_.toPairs(_.omit(partUpdated, keysToOmit)), _.toPairs(assetToUpdate.get({ plain: true })), _.isEqual),
        );

        if (
          diff.hasOwnProperty("production_date_gmt") &&
          new Date(assetToUpdate.production_date_gmt).getTime() === new Date(partUpdated.production_date_gmt).getTime()
        ) {
          // Date in fabric can be 2020-11-25T10:39:33.000Z while being 2020-11-25 10:39:33.000000 in postgres
          Logger.warn("Not updating production_date_gmt as SQL version and new version are the same after parsing!");
        ...
        for (const value of notInDB) {
          if (typeof value === "string") {
            await this.addRelationship(
              mspIDFromJWT,
              partUpdated.serial_number_customer,
              value,
              assetToUpdate["mspid"],
              relationshipExtras.hasOwnProperty(value) && relationshipExtras[value].hasOwnProperty("child_mspid")
                ? relationshipExtras[value]["child_mspid"]
                : "",
              relationshipExtras.hasOwnProperty(value) && relationshipExtras[value].hasOwnProperty("transfer_status")
                ? relationshipExtras[value]["transfer_status"]
                : 0,
            );
          }
        }

        for (const value of notInAsset) {
          if (typeof value === "string") {
            await this.deleteRelationship(partUpdated.serial_number_customer, value, assetToUpdate["mspid"]);
          }
        }
      } else {
  ...
  async addRelationship(
    mspIDFromJWT: string,
    parentSerialNumberCustomer: string,
    childSerialNumberCustomer: string,
    parentMSPID: string = "",
    childMSPID: string = "",
    transferStatus: number = 0,
    retries: number = 0,
    lastRetry: Date = new Date(),
  ) {
    const connectionPool = await this.connectAndSync(mspIDFromJWT);
   ...
   * @param pagination
   */
  async buildTreeQuery(
    whereConditionStringParent: string,
    maxChildLevel: number = 0,
    pagination: number = 0,
    fields: any = RichQuerySQL.defaultFields.map((field: any) => Strings.camelCaseToSnakeCase(field)),
  ) {
  ...
  async cleanFilter(filter: any) {
    ...
    const assetColumns = getAssetModelDefinition(AssetModel);
   ...
   */
  async getRelationshipsByTransferStatus(
    transferStatus: number | Array<number>,
    mspIDFromJWT: string,
    limit: number = 100,
    orderRandom: boolean = false,
  ) {
    Logger.debug(
      `[${mspIDFromJWT}] getAssetByTransferStatus in OffChainDBClient with status ${transferStatus}, limit ${limit} and orderRandom ${orderRandom}`,
    );
  ...
  async getRelationshipsNotInFabric(mspIDFromJWT: string, limit: number = 100) {
    ...
    const newRelationshipsNotInFabric = await connectionPool.pool.query(
      queryString,
    ...
    const mediumRelationshipsNotInFabric = await connectionPool.pool.query(
      queryString,
      this.createRawQueryOptionsWithReplacement(whereCondition.replacements),
    );
   ...
   * @memberof OffChainDBClient
   */
  async getParentFromRelationshipByChildSerialNumberCustomer(childSerialNumberCustomer: string, mspIDFromJWT: string) {
    Logger.info(`[${mspIDFromJWT}] getParentFromRelationshipByChildSerialNumberCustomer called`);
    ...
    const filter = {
      selector: {
        child_serial_number_customer: {
          whereClause: " = :child_serial_number_customer ",
          replacement: {
            child_serial_number_customer: childSerialNumberCustomer,
          },
        },
  ...
  async getRelationshipsByParentAndStatus(parentSerialNumberCustomer: string, transferStatus: number | Array<number>, mspIDFromJWT: string) {
    Logger.debug(
      `[${mspIDFromJWT}] getRelationshipsByParentAndStatus in OffChainDBClient with parentSerialNumberCustomer ${parentSerialNumberCustomer} and transferStatus ${transferStatus}`,
    );
   ...
   * @param mspIDFromJWT
   */
  async updateRelationshipStatusWithMspID(
    mspIDFromJWT: string,
    childSerialNumberCustomer: string,
    transferStatus: number = 0,
    childMspID: string = " ",
    ...
    Logger.info(
      `[${mspIDFromJWT}] updateRelationshipStatusWithMspID of ${childSerialNumberCustomer} to status ${transferStatus} and mspID ${childMspID}`,
    );

    const connectionPool = await this.connectAndSync(mspIDFromJWT);

    const queryString = `UPDATE relationships SET  transfer_status = ${transferStatus}, child_mspid = '${childMspID}', last_retry = '${new Date().toISOString()}', retries = retries + 1 WHERE child_serial_number_customer = '${childSerialNumberCustomer}' `;
    Logger.debug(`[${mspIDFromJWT}] Executing query: ${queryString}`);
    await connectionPool.pool.query(queryString, {
      plain: false,
      raw: true,
  ...
  async updateRelationshipStatus(mspIDFromJWT: string, childSerialNumberCustomer: string, transferStatus: number = 0) {
    ...
    const queryString = `UPDATE relationships SET transfer_status = ${transferStatus}, last_retry = '${new Date().toISOString()}', retries = retries + 1 WHERE child_serial_number_customer = '${childSerialNumberCustomer}' `;
    Logger.debug(`[${mspIDFromJWT}] Executing query: ${queryString}`);
    await connectionPool.pool.query(queryString, {
      plain: false,
      raw: true,
