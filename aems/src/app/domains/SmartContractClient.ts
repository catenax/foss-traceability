...
import { getMSPFromCustomerOneID, returnFunction } from "../modules/helper-functions/Helper";
...
export default class SmartContractClient extends Gateway {
  ...
  async processTransaction(tx: string, mspIDFromJWT: string, payload: any[], type: string = "eval", pagination: number = 0) {
    ...
    await this.connectToMultipleChannels(defaults.hlfDefaultMspID, [defaults.channelName], chaincodeName);
    return await this.connectionsWalk(this.connections[defaults.hlfDefaultMspID], tx, mspIDFromJWT, payload, type, pagination);
  }
  /**
  ...
  async getContractConnection(channelName: string[] = ["wolf-channel"], chaincodeName: string = "partchainccw") {
    ...
    await this.connectToMultipleChannels(mspID, channelName, chaincodeName);
  ...
  async connectionsWalk(connections: any, tx: string, mspIDFromJWT: string, payload: any[], type: string, pagination: number) {
    ...
    const promisesConnections = connections.map(async (connection: any) => {
      Logger.info(
        `[${mspIDFromJWT}] Executing transaction ${tx} for mspID ${mspIDFromJWT} and channel "${
      ...
      for (let key in payload) {
        ...
        const buffer = await SmartContractClient.executeTransaction(connection.channel, connection.contract, tx, mspIDFromJWT, payload[key], type);
    ...
    if (type === "submit") {
      ...
      switch (tx) {
        ...
        case "exchangeAssetInfo":
          Logger.debug(`[${mspIDFromJWT}] processTransaction: ${tx} `);
          const providerMSP = payload.targetOrgMSP.split("-");
          mspID = providerMSP[0];
          break;
        case "createAsset":
        case "updateAsset":
      ...
      return await contract.evaluateTransaction(tx, tx, Payload.removeExtraComma(JSON.stringify(payload)));
    }
  ...
  async upsertAsset(asset: AssetList, mspIDFromJWT: string) {
    ...
    await Promise.all(
      asset.map(async (singleAsset: Asset): Promise<any> => {
        ...
        if (alreadyStoredCheck.status === 200) {
          ...
          //TODO Enable this again after the PoC
          // singleAsset.componentsSerialNumbers = [
          //   ...new Set([...singleAsset.componentsSerialNumbers, ...alreadyStoredCheck.data.componentsSerialNumbers]),
          // ];

          Logger.info(`[${mspIDFromJWT}] singleAsset before IsAsset Current Check ${JSON.stringify(singleAsset.customFields)}`);
          Logger.info(`[${mspIDFromJWT}] singleAsset  Current Check ${JSON.stringify(alreadyStoredCheck.data.customFields)}`);
          singleAsset["customFields"]["businessPartnerName"] =
            singleAsset.customFields.hasOwnProperty("businessPartnerName") && singleAsset.customFields.businessPartnerName !== null
              ? singleAsset.customFields.businessPartnerName
              : singleAsset.customFields.manufacturerOneId;
          singleAsset["customFields"]["customerPartnerName"] =
            singleAsset.customFields.hasOwnProperty("customerPartnerName") && singleAsset.customFields.customerPartnerName !== null
              ? singleAsset.customFields.customerPartnerName
              : singleAsset.customFields.customerOneId;

          Logger.info(`[${mspIDFromJWT}] singleAsset after IsAsset Current Check ${JSON.stringify(singleAsset.customFields)}`);
          ...
          if (
            nothingToUpdateCheck.hasOwnProperty("data") &&
            nothingToUpdateCheck["data"].hasOwnProperty("isCurrent") &&
            nothingToUpdateCheck.data.isCurrent
          ) {
          ...
          singleAsset.mspID = mspIDFromJWT;
    ...
    let [responseUpdate, responseStore] = await Promise.all([
      this.updateAsset(assetsToUpdate, mspIDFromJWT),
      this.storeAsset(assetsToStore, mspIDFromJWT),
    ]);
    const returnObject: any = {
      status:
        responseStore.status == 200 && responseUpdate.status == 200
          ? 200
          : responseUpdate.status == 200
          ? responseUpdate.status
          : responseStore.status,
      data: [
        ...Iterable.create(responseUpdate.data ? responseUpdate.data : []),
        ...Iterable.create(responseStore.data ? responseStore.data : []),
        ...responseNothingToUpdate,
      ],
    ...
    if (responseUpdate.hasOwnProperty("error") || responseStore.hasOwnProperty("error")) {
      returnObject["error"] = [
        ...(responseUpdate.hasOwnProperty("error") ? responseUpdate.error : []),
        ...(responseStore.hasOwnProperty("error") ? responseStore.error : []),
      ];
    }
    return returnObject;
  }
   ...
   */
  async updateAsset(assets: AssetList, mspIDFromJWT: string, createTransactionRecord: boolean = false, isQualityAlert: boolean = false) {
    ...
    Logger.debug(`[${mspIDFromJWT}]updateAsset: Ready for transaction in updateAsset `);
    let response = await this.processTransaction("updateAsset", mspIDFromJWT, Iterable.create(ownAssets), "submit");
    Logger.debug(`[${mspIDFromJWT}]updateAsset: response of updateAsset: ${JSON.stringify(response)}`);
    ...
    for (let ownSingleAsset of ownAssets) {
      ...
      if (!isQualityAlert) {
        //TODO: think about the field [asset.data.qualityStatus]
        if (adjustedAsset.customFields.hasOwnProperty("qualityAlert")) {
          delete adjustedAsset["customFields"]["qualityAlert"];
        }
        if (adjustedAsset.customFields.hasOwnProperty("qualityType")) {
          delete adjustedAsset["customFields"]["qualityType"];
        }
        if (adjustedAsset.customFields.hasOwnProperty("alertID")) {
          delete adjustedAsset["customFields"]["alertID"];
        }
        if (adjustedAsset.customFields.hasOwnProperty("eventOrigin")) {
          delete adjustedAsset["customFields"]["eventOrigin"];
        }
        if (adjustedAsset.customFields.hasOwnProperty("eventType")) {
          delete adjustedAsset["customFields"]["eventType"];
        }
        if (adjustedAsset.customFields.hasOwnProperty("eventOriginCompany")) {
          delete adjustedAsset["customFields"]["eventOriginCompany"];
        }
        if (adjustedAsset.customFields.hasOwnProperty("eventTargetCompany")) {
          delete adjustedAsset["customFields"]["eventTargetCompany"];
        }
      }

      const exchangeResponse = isQualityAlert
        ? await this.assetExchangeForQualityAlert(adjustedAsset, ownSingleAsset, createTransactionRecord, mspIDFromJWT)
        : await this.assetUpdateExchangeNormal(adjustedAsset, ownSingleAsset, createTransactionRecord, mspIDFromJWT);

      Logger.info(`[${mspIDFromJWT}]updateAsset:  response of exchangeResponse: ${JSON.stringify(exchangeResponse)}`);
    }

    // Returning the response for update asset
    return await Response.processResponse(
      [
        ...Iterable.create(foreignAssetsResponse.data.length > 0 ? foreignAssetsResponse : []),
        ...Iterable.create(response.length > 0 ? response : []),
      ],
      true,
      mspIDFromJWT,
      true,
  ...
  async getAssetEventDetail(serialNumberCustomer: any, mspIDFromJWT: string) {
    ...
    const filter = { serialNumberCustomer: serialNumberCustomer, mspID: mspIDFromJWT };
   ...
   *
   * @param {*} adjustedAsset
   * @param {*} ownSingleAsset
   * @param {string} mspIDFromJWT
   * @memberof SmartContractClient
   */
  async assetExchangeForQualityAlert(adjustedAsset: any, ownSingleAsset: any, createTransactionRecord: any, mspIDFromJWT: string) {
    Logger.info(`Called assetExchangeForQualityAlert`);

    const offChainDBClient = new OffChainDBClient();
    Logger.info(`assetExchangeForQualityAlert:  adjustedAsset = ${JSON.stringify(adjustedAsset)}`);
    const eventTargetCompany = adjustedAsset["customFields"]["customerOneId"];
    const mspID = await getMSPFromCustomerOneID(eventTargetCompany);
    // Only  share allowed child serialNumberCustomer
    ...
    adjustedAsset.componentsSerialNumbers = childRelationshipsForThisAsset
      .filter((relationship: any) => relationship.child_mspid === mspID)
      .map((relationship: any) => relationship.child_serial_number_customer);

    Logger.info(`assetExchangeForQualityAlert: eventTargetCompany = ${eventTargetCompany} &&  mspID = ${mspID}`);
    if (mspID != "NotFound") {
      const exchangeAssetResponse = await this.exchangeAsset(mspID, adjustedAsset.serialNumberCustomer, JSON.stringify(adjustedAsset), mspIDFromJWT);
      Logger.info(`[${mspIDFromJWT}] response of exchangeAsset =  ${JSON.stringify(exchangeAssetResponse)}`);
      if (exchangeAssetResponse.status !== 200) {
        Logger.info(`[${mspIDFromJWT}] exchange asset mspID ${mspID}`);
        if (exchangeAssetResponse.status === 403) {
          //TODO should this throw an error or not?
          Logger.warn(
            `[${mspIDFromJWT}] Error when trying to exchange updated Asset with mspID ${mspID} since ACL is INACTIVE: ${JSON.stringify(
              exchangeAssetResponse,
            )}`,
          );
        } else {
      ...
      return await offChainDBClient.updateAsset(Iterable.create(ownSingleAsset), mspIDFromJWT, Object.create({}), createTransactionRecord);
    } else {
      await offChainDBClient.updateAsset(Iterable.create(ownSingleAsset), mspIDFromJWT, Object.create({}), createTransactionRecord);
      return await returnFunction(`MSPID  ${mspID} is not part of PartChain. So, we don´t do the exchange  `, ResponseStatus.PERMISSION_DENIED);
    }
  }
   ...
   * @memberof SmartContractClient
   */
  async assetUpdateExchangeNormal(adjustedAsset: any, ownSingleAsset: any, createTransactionRecord: any, mspIDFromJWT: string) {
    Logger.debug(`Called assetUpdateExchangeNormal`);

    const offChainDBClient = new OffChainDBClient();

    Logger.info(`assetUpdateExchangeNormal:  adjustedAsset = ${JSON.stringify(adjustedAsset)}`);
    // Get relationships of this asset where status is shared
    const relationships = await offChainDBClient.getRelationshipsByParentAndStatus(
      ownSingleAsset.serialNumberCustomer,
      [RelationshipStatusType.parentShared, RelationshipStatusType.childShared],
      mspIDFromJWT,
    );
    ...
    const parentMspIDs: Array<any> =
      parentRelationshipsForThisAsset.hasOwnProperty("parents") && parentRelationshipsForThisAsset.parents.length > 0
        ? [...new Set(parentRelationshipsForThisAsset.parents.map((parent: any) => parent.mspid))]
        : [];
    ...
    for (let mspID of uniqueMspIDs) {
      ...
      adjustedAsset.componentsSerialNumbers = childRelationshipsForThisAsset
        .filter((relationship: any) => relationship.child_mspid === mspID)
        .map((relationship: any) => relationship.child_serial_number_customer);
      Logger.debug(`[${mspIDFromJWT}] assetUpdateExchangeNormal: exchange asset mspID ${mspID}`);
      Logger.debug(`assetUpdateExchangeNormal: adjusted Asset = ${JSON.stringify(adjustedAsset)}`);
      const exchangeAssetResponse = await this.exchangeAsset(mspID, adjustedAsset.serialNumberCustomer, JSON.stringify(adjustedAsset), mspIDFromJWT);
      Logger.debug(`[${mspIDFromJWT}] assetUpdateExchangeNormal: response of exchangeAsset =  ${JSON.stringify(exchangeAssetResponse)}`);
      if (exchangeAssetResponse.status !== 200) {
        Logger.debug(`[${mspIDFromJWT}] assetUpdateExchangeNormal: exchange asset mspID ${mspID}`);
        if (exchangeAssetResponse.status === 403) {
          //TODO should this throw an error or not?
  ...
  protected getChildrenSerialNumbers(collection: Array<any>): Array<string> {
    ...
    return (Array.isArray(collection) ? collection : [collection]).reduce((acc: Array<string>, object) => {
      ...
      if (Array.isArray(object.componentsSerialNumbers)) {
        object.componentsSerialNumbers.forEach((serialNumber: any) => acc.push(serialNumber));
      }
      return acc;
    }, []);
  }
      ...
      if (Array.isArray(asset.componentsSerialNumbers) && asset.componentsSerialNumbers.length > 0) {
        ...
        if (unavailableChildren.length > 0) {
          throw new customErrors.BadRequestError(
            `These children of asset ${
              asset.serialNumberCustomer
            } already have a different parent and are not of type BATCH: ${unavailableChildren.toString()}`,
          );
        }
      }
    }
