  ...
  try {
    ...
    for (let identity of Object.keys(hlfIdentities)) {
      ...
      if (hlfIdentities[identity].hasOwnProperty("HLF_MANAGED_IDENTITY_MSP_ID")) {
        ...
        mspIDs = [mspIDs, ...hlfIdentities[identity]["HLF_MANAGED_IDENTITY_MSP_ID"]];
      }
      for (let mspIDFromJWT of mspIDs) {
        if (mspIDFromJWT === mspID) {
          ...
          if (assetEventDetails.hasOwnProperty("data") && assetEventDetails.data.hasOwnProperty("serialNumberCustomer")) {
            await processRequestAssetEvent(assetEventDetails, mspIDFromJWT);
          } else if (assetEventDetails.hasOwnProperty("data") && assetEventDetails.data.hasOwnProperty("investigationID")) {
...
const processRequestAssetEvent = async (assetEventDetails: any, mspIDFromJWT: string) => {
  ...
  const validate_response = await client.validateAsset(asset, mspIDFromJWT);
  ...
  const flaggedComponentSerialNumbers: Array<string> = asset.childSerialNumberCustomer
    .filter((r: any) => r.flagged)
    .map((r: any) => r.serialNumberCustomer);
  ...
  if (validate_response.data.result) {
      ...
      if (childDetails.status === 200) {
        ...
        if (exchangeAssetResponse.status === 200) {
          ...
          await offChainDBClient.updateRelationshipStatus(
            mspIDFromJWT,
            childDetails.serialNumberCustomer,
            RelationshipStatusType.childExchangeFailure,
          );
        }
      } else {
    ...
    Logger.warn(
      `[${mspIDFromJWT}] Parent component verification failed, storing Asset anyway with status ${RelationshipStatusType.parentHashValidationFailure}, but not sharing anything with parent!`,
    );
    const relationshipExtras = Object.create({});
    for (let child of asset.componentsSerialNumbers) {
      relationshipExtras[child] = {
...
const processInvestigation = async (eventDetails: any, mspIDFromJWT: string) => {
  ...
  if (investigationPublicDetails[0].hasOwnProperty("data") && investigationPublicDetails[0].data.hasOwnProperty("participatingOrgs")) {
