...
export default async function createContractListener() {
  ...
  Logger.info(
    `Setting up chaincode listener for channel ${defaults.channelName}, using default mspID ${defaults.hlfDefaultMspID} and chaincode ${chaincodeName}`,
  );

  return client
    .getContractConnection([defaults.channelName], chaincodeName)
    .then((response: any) => {
