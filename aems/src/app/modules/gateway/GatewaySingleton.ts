...
export default class GatewaySingleton {
   ...
   */
  async setup() {
    let hlfIdentitiesBuffer = readFileSync(defaults.hlfIdentitiesFilePath);
  ...
  static getConnectionProfile(networkProfile: string): string | object {
    return safeLoad(readFileSync(networkProfile, "utf8"));
  }
  ...
  validateHLFIdentityFile(mspID: string) {
    ...
    const properties: Array<string> = [
      "HLF_NETWORK_PROFILE",
      "HLF_NETWORK_CHAINCODE_ID",
      "HLF_IDENTITY_CERT_PATH",
      "HLF_IDENTITY_KEY_PATH",
      "HLF_IDENTITY_CONFIG_PATH",
      "HLF_IDENTITY_USERNAME",
      "HLF_IDENTITY_MSP_ID",
      "HLF_IDENTITY_MSP_PATH",
    ];
