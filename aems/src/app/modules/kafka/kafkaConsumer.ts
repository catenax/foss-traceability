  ...
  async consume() {
    ...
    for (let identity of Object.keys(hlfIdentities)) {
      ...
      for (let mspID of mspIDs) {
        if (!topics.includes(mspID)) {
          ...
          //await this.consumer.subscribe({ topic, fromBeginning: true }); // In case of replay from beginning
    ...
    this.consumer.run({
      eachMessage: async ({ topic, partition, message }: any) => {
        Logger.debug(`[KAFKA] : consume - topic = ${topic}  partition = ${partition}  message = ${JSON.stringify(message)}`);
        ...
        try {
            ...
            case response.status >= 200 && response.status < 300:
              ...
              this.producer
                .send({
                  ...
                  messages: [{ value: JSON.stringify(ingestStatsMessage) }],
                })
                .then((r: any) =>
                  Logger.debug(`[KAFKA] Send message ${JSON.stringify(ingestStatsMessage)} successfully to ingest stats topic: ${JSON.stringify(r)}`),
                )
          ...
          Logger.error(`[KAFKA] error ${JSON.stringify(e)} `);
          ...
          switch (e.constructor) {
            ...
            case BadRequestError:
              ...
              this.producer
                .send({
                  ...
                  messages: [{ value: JSON.stringify(failedIngestStatsMessage) }],
                })
                .then((r: any) =>
                  Logger.info(
                    `[KAFKA] Send message ${JSON.stringify(failedIngestStatsMessage)} successfully to failed ingest topic: ${JSON.stringify(r)}`,
                  ),
                )
            ...
            default:
              ...
              Logger.error(
                `[KAFKA] [${topic}] Problem when calling storeAsset ${JSON.stringify(e)} with ${JSON.stringify(asset)}.` +
