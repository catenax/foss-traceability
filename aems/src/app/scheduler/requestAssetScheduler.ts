  ...
  cron.schedule(defaults.cronjob.status.notInFabric.schedule, async () => {
    ...
    for (let identity of Object.keys(hlfIdentities)) {
      ...
      for (let mspIDfromJWT of mspIDs) {
        ...
        let notInFabricRelationships: any = await offChainDBClient.getRelationshipsNotInFabric(
          mspIDfromJWT,
          defaults.cronjob.status.notInFabric.limit,
        );

        Logger.debug(
          `[${mspIDfromJWT}] List of assets with status ${RelationshipStatusType.notInFabric}: ${JSON.stringify(notInFabricRelationships)} `,
        );
        Logger.info(
          `[${mspIDfromJWT}] Scheduler: ${notInFabricRelationships.length}  Assets with status ${RelationshipStatusType.notInFabric} (maximum: ${defaults.cronjob.status.notInFabric.limit})`,
        );

        // 3. check if the child is part of the PartChain network by now
        await Promise.all(
          notInFabricRelationships.map(async (childComponent: any) => {
            ...
            if (childComponentPublicDetails.status === 200) {
              ...
              Logger.info(
                `[${mspIDfromJWT}] Component ${childComponent.child_serial_number_customer} found in PartChain, changing its status to ${RelationshipStatusType.childInPublicLedger}`,
              );
              ...
              await offChainDBClient.updateRelationshipStatusWithMspID(
                mspIDfromJWT,
                childComponent.child_serial_number_customer,
                RelationshipStatusType.childInPublicLedger,
                childComponentPublicDetails.mspID,
              );
            } else {
              await offChainDBClient.updateRelationshipStatus(
                mspIDfromJWT,
                childComponent.child_serial_number_customer,
                RelationshipStatusType.notInFabric,
              );
            }
          }),
        );
  ...
  cron.schedule(defaults.cronjob.status.parentShared.schedule, async () => {
    ...
    for (let identity of Object.keys(hlfIdentities)) {
      ...
      for (let mspIDfromJWT of mspIDs) {
        ...
        let parentSharedRelationships: any = await offChainDBClient.getRelationshipsByTransferStatus(
          RelationshipStatusType.parentShared,
          mspIDfromJWT,
          defaults.cronjob.status.parentShared.limit,
        );
        ...
        Logger.debug(
          `[${mspIDfromJWT}] Scheduler: List of assets with status ${RelationshipStatusType.parentShared}: ${JSON.stringify(
            parentSharedRelationships,
          )} `,
        );
        Logger.info(
          `[${mspIDfromJWT}] Scheduler:  ${parentSharedRelationships.length} Assets with status ${RelationshipStatusType.parentShared}  (maximum: ${defaults.cronjob.status.parentShared.limit})`,
        );

        // Filter relationships with are only recently updated to parentShared (less than 60 minutes)
        parentSharedRelationships = parentSharedRelationships.filter(
          (rel: any) => new Date().valueOf() - new Date(rel.last_retry).valueOf() > 3600000,
        );
        ...
        Logger.info(
          `Scheduler for status ${RelationshipStatusType.parentShared} is done with this round! Changed ${parentSharedRelationships.length} assets back to status ${RelationshipStatusType.childInPublicLedger} from ${RelationshipStatusType.parentShared}, as no child was shared`,
        );
      }
    }
  ...
  cron.schedule(defaults.cronjob.status.unknown.schedule, async () => {
    ...
    for (let identity of Object.keys(hlfIdentities)) {
      ...
      for (let mspIDfromJWT of mspIDs) {
        ...
        let unknownRelationships: any = await offChainDBClient.getRelationshipsByTransferStatus(
          RelationshipStatusType.unknown,
          mspIDfromJWT,
          defaults.cronjob.status.unknown.limit,
          true,
        );
        ...
        Logger.info(
          `[${mspIDfromJWT}] Scheduler: ${unknownRelationships.length} assets with status ${RelationshipStatusType.unknown} (maximum: ${defaults.cronjob.status.unknown.limit})`,
        );

        // 3. check if the child is part of the PartChain network
        await Promise.all(
          unknownRelationships.map(async (childComponent: any) => {
            ...
            if (childComponentDetails.status === 200) {
              ...
              if (childComponentDetails.mspID === mspIDfromJWT) {
                ...
                await offChainDBClient.updateRelationshipStatusWithMspID(
                  mspIDfromJWT,
                  childComponent.child_serial_number_customer,
                  RelationshipStatusType.childShared,
                  childComponentDetails.mspID,
                );
              } else {
                await offChainDBClient.updateRelationshipStatusWithMspID(
                  mspIDfromJWT,
        ...
        let childrenInFabric: any[] = await offChainDBClient.getRelationshipsByTransferStatus(
          [RelationshipStatusType.childInPublicLedger, RelationshipStatusType.updatePending],
          mspIDfromJWT,
          defaults.cronjob.status.childInPublicLedger.limit,
        );
        Logger.info(
          `[${mspIDfromJWT}] Scheduler: ${childrenInFabric.length} Assets with status ${RelationshipStatusType.childInPublicLedger} or ${RelationshipStatusType.updatePending} (maximum: ${defaults.cronjob.status.childInPublicLedger.limit})`,
        );
        Logger.debug(
          `[${mspIDfromJWT}] Relationships with status ${RelationshipStatusType.childInPublicLedger} or ${
            RelationshipStatusType.updatePending
          } ${JSON.stringify([].concat(...childrenInFabric))}`,
        );
        ...
        await Promise.all(
          uniqueParentSerialNumberCustomer.map(async (parentSerialNumberCustomer: any) => {
            // GetParent
            ...
            if (parentAsset.status === 200) {
              ...
              const unsharedRelationshipsForThisParent = childrenInFabric.filter(
                relationship => relationship.parent_serial_number_customer === parentSerialNumberCustomer,
              );
              ...
              for (let mspID of uniqueMspIDs) {
                let requestAsset = {
                  ...
                  targetOrgMSP: mspID, //mspID of the supplier
                  mspID: mspIDfromJWT,
                  ...
                  childSerialNumberCustomer: relationshipsForThisParent
                ...
                if (requestAssetResponse.status === 200) {
                  for (let rel of unsharedRelationshipsForThisParent.filter(relationship => relationship.child_mspid === mspID)) {
                    await offChainDBClient.updateRelationshipStatus(
                      mspIDfromJWT,
                      rel.child_serial_number_customer,
                      RelationshipStatusType.parentShared,
                    );
                  }
                  ...
                  for (let rel of unsharedRelationshipsForThisParent.filter(relationship => relationship.child_mspid === mspID)) {
                    await offChainDBClient.updateRelationshipStatus(
                      mspIDfromJWT,
                      rel.child_serial_number_customer,
                      RelationshipStatusType.requestAssetNotAllowed,
                    );
                  }
                } else {
  ...
  cron.schedule(defaults.cronjob.status.requestAssetNotAllowed.schedule, async () => {
    ...
    for (let identity of Object.keys(hlfIdentities)) {
      ...
      for (let mspIDfromJWT of mspIDs) {
        ...
        let requestAssetNotAllowedRelationships: any = await offChainDBClient.getRelationshipsByTransferStatus(
          RelationshipStatusType.requestAssetNotAllowed,
          mspIDfromJWT,
          defaults.cronjob.status.requestAssetNotAllowed.limit,
          true,
        );

        Logger.debug(
          `[${mspIDfromJWT}] List of assets with status ${RelationshipStatusType.requestAssetNotAllowed}: ${JSON.stringify(
            requestAssetNotAllowedRelationships,
          )} `,
        );
        Logger.info(
          `[${mspIDfromJWT}] Scheduler: ${requestAssetNotAllowedRelationships.length}  Assets with status ${RelationshipStatusType.requestAssetNotAllowed} (maximum: ${defaults.cronjob.status.requestAssetNotAllowed.limit})`,
        );
        ...
        if (ACL.status === 200) {
          ...
          await Promise.all(
            requestAssetNotAllowedRelationships.map(async (childComponent: any) => {
              if (_.includes(relevantMspIDs, childComponent.child_mspid)) {
                Logger.info(
                  `[${mspIDfromJWT}] Asset ${childComponent.child_serial_number_customer} can now be requested, changing its status from status ${RelationshipStatusType.requestAssetNotAllowed} to ${RelationshipStatusType.unknown}`,
                );
                await offChainDBClient.updateRelationshipStatus(
                  mspIDfromJWT,
                  childComponent.child_serial_number_customer,
