...
import QualityAlertScheduler from "./app/scheduler/qualityAlertScheduler";
...
import QualityAlertMgmtAPI from "./app/routes/QualityAlertAPI";
import KafkaConsumer from "./app/modules/kafka/kafkaConsumer";
import AlertConsumer from "./app/modules/kafka/QualityAlertConsumer";
import AlertHistoryConsumer from "./app/modules/kafka/AlertHistoryConsumer";
...
if (cluster.isMaster) {
  ...
  if (defaults.cronjob.enabled) {
    ...
    Scheduler().catch((error: any) => {
      Logger.error(`Error in setting up the Scheduler ${error}`);
      throw new Error(`Error in setting up the Scheduler ${error}`);
    });

    Logger.info(`Starting the QualityAlert Scheduler`);
    QualityAlertScheduler().catch((error: any) => {
      Logger.error(`Error in setting up the QualityAlert Scheduler ${error}`);
      throw new Error(`Error in setting up the QualityAlert Scheduler ${error}`);
    });
  }
  ...
  if (defaults.kafka.enabled) {
    Logger.info(`[KAFKA] quality alert consumer is enabled!`);
    const alertConsumer = new AlertConsumer();
    alertConsumer.alertConsume();

    const alertHistoryConsumer = new AlertHistoryConsumer();
    alertHistoryConsumer.alertHistoryConsume();
  }
  ...
  new App({
    routes: [SmartContractAPI, AccessMgmtAPI, QualityAlertMgmtAPI],
    swagger: {
      host: defaults.host,
      apis: [
