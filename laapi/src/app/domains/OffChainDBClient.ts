...
import QualityAlertModel from "../models/QualityAlert"
...
export default class OffChainDBClient extends SQLClient {
     ...
     * @param mspID
     */
    async connectAndSync(mspID: any): Promise<{ pool: Sequelize; assetModel: any; relationshipModel: any; qualityAlertModel: any; }> {
        ...
        const qualityAlertModel = await this.initModel(QualityAlertModel, connectionPool);
        await qualityAlertModel.sync();
        return {pool: connectionPool, assetModel: assetModel, relationshipModel: relationshipModel, qualityAlertModel};
    }
     ...
     * @param fields
     */
    async getParent(serialNumberCustomers: string | Array<string>, mspIDFromJWT: string,
            fields: Array<string> = RichQuerySQL.defaultFields.map(((field: string) => Strings.camelCaseToSnakeCase(field)))) {
        Logger.info(`[${mspIDFromJWT}] getAssetParent in OffChainDBClient`);
        const connectionPool = await this.connectAndSync(mspIDFromJWT);
        const serialNumberList = Iterable.create(serialNumberCustomers);
        ...
        const whereConditionString = `IN (:serialNumberList)`
        const queryString = `SELECT parents
                   FROM (
                            SELECT a.*,
                                   COALESCE(jsonb_agg(r.child_serial_number_customer)
                                            FILTER ( WHERE NOT (r.child_serial_number_customer = 'null')), ' []
     ...
     * @param mspIDFromJWT
     */
    async getQualityAlert(startDate: Date, endDate: Date, mspIDFromJWT: string) {
        Logger.debug(`[${mspIDFromJWT}] getQualityAlert in OffChainDBClient}`);
        ...
        const currentGetQualityAlert = await connectionPool.qualityAlertModel.findAll({
            where: filter,
        });
        Logger.info(`[${mspIDFromJWT}] Executing query: ${currentGetQualityAlert}`);

        return currentGetQualityAlert;
    }
}
