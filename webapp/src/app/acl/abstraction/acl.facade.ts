...
export class AclFacade {
  ...
  public setAcls(): void {
    ...
    this.aclService.getACL().subscribe(
      ...
      error => this.aclState.setAcls({ error }),
    );
  }
  ...
  public requestAccess(targetOrg: string, comment: string): void {
    ...
    this.aclService.requestAccess(targetOrg, comment).subscribe(
      (requestedAcl: ACLResponse) => {
        if (!this.sharedService.isEmpty(requestedAcl.data)) {
          this.aclState.updateAcls({ data: Object.values(requestedAcl.data.ACL) }, requestedAcl.data.owner);
          this.updateAclBadge(requestedAcl.data.ACL);
        }
      },
      error => this.aclState.setAcls({ error }),
    );
  ...
  public removeAclAccess(targetOrg: string, comment: string): void {
    ...
    this.aclService.removeAccess(targetOrg, comment).subscribe(
      (removedAcl: ACLResponse) => {
        if (!this.sharedService.isEmpty(removedAcl.data)) {
          this.aclState.updateAcls({ data: Object.values(removedAcl.data.ACL) }, removedAcl.data.owner);
          this.updateAclBadge(removedAcl.data.ACL);
        }
      },
      error => this.aclState.setAcls({ error }),
    );
  ...
  public acceptAclAccess(targetOrg: string, comment: string): void {
    ...
    this.aclService.acceptRequest(targetOrg, comment).subscribe(
      (acceptedAcl: ACLResponse) => {
        if (!this.sharedService.isEmpty(acceptedAcl.data)) {
          this.aclState.updateAcls({ data: Object.values(acceptedAcl.data.ACL) }, acceptedAcl.data.owner);
          this.updateAclBadge(acceptedAcl.data.ACL);
        }
      },
      error => this.aclState.setAcls({ error }),
    );
  ...
  public denyAclAccess(targetOrg: string, comment: string): void {
    ...
    this.aclService.denyRequest(targetOrg, comment).subscribe(
      (deniedAcl: ACLResponse) => {
        if (!this.sharedService.isEmpty(deniedAcl.data)) {
          this.aclState.updateAcls({ data: Object.values(deniedAcl.data.ACL) }, deniedAcl.data.owner);
          this.updateAclBadge(deniedAcl.data.ACL);
        }
      },
      error => this.aclState.setAcls({ error }),
    );
