...
export class AuthService {
  ...
  public getUserData(): {
    ...
    return {
      ...
      auth_time: tokenParsed.auth_time ? tokenParsed.auth_time : '',
  ...
  /**
   * Get url depending on the provided mspid
   *
   * @return {{ laapi: string; aems: string }}
   * @memberof AuthService
   */
  public getUrl(): { laapi: string; aems: string } {
    const { mspid } = this.getUserData();
    const KeycloakRealm = mspid ? mspid : realm[1];
    return KeycloakRealm.toLocaleLowerCase() === 'bmw'
      ? { laapi: '@BMWLAAPI', aems: '@BMWAEMS' }
      : { laapi: '@TAASLAAPI', aems: '@TAASAEMS' };
  }
