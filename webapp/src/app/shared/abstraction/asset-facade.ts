...
export class AssetFacade {
  ...
  public openQualityAlertModal(asset: Asset): void {
    const dialogRef = this.dialog.open(QualityAlertCreateComponent, {
      width: '400px',
    ...
    dialogRef.afterClosed().subscribe((data: { qualityType: string; selectedSerialNumbers: Asset[] }) => {
      if (data) {
        const serialNumbers = data.selectedSerialNumbers.map(value => value.serialNumberCustomer);
        this.qualityAlertFacade.createQualityAlert(data.qualityType.toLocaleUpperCase(), true, serialNumbers);
      }
    });
  }
